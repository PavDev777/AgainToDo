...
const Tasks = ({
  ...
  const editTasks = (task) => {
    const newTask = window.prompt("Название задачи", task.text);
    if (newTask) {
      editTask(list.id, { id: task.id, text: newTask });
      axios
  ...
  const onChangeChekbox = (taskId, event) => {
    onCompleteTask(list.id, taskId, event.target.checked)
  };

  return (
    <div className={Styled.tasks}>
      <div className={Styled.tasks__title}>
      ...
      {list.tasks &&
        list.tasks.map((task) => (
          <div key={task.id} className={Styled.tasks__items}>
            <div className={Styled.checkbox}>
              <input
                id={`id = ${task.id}`}
                type="checkbox"
                onChange={(event) => onChangeChekbox(task.id, event)}
            ...
            <p> {task.text} </p>
            <div className={Styled.tasks__items_row_actions}>
              <div onClick={() => editTasks(task)}>
                <svg
                  width="11"
                  height="8"
                  viewBox="0 0 11 8"
